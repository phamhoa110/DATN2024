...
public class CorgiController : MonoBehaviour 
{
	...
	private void CastRaysBelow()
	{
		...
		if (hitConnected)
		{
			...
			{
				float newPosY=movingPlatform._boxCollider.bounds.max.y+ColliderSize.y/2+RayOffset*transform.localScale.y-_boxCollider.offset.y*transform.localScale.y;
				//_transform.Translate(movingPlatform.CurrentSpeed*Time.deltaTime);
				_transform.position=new Vector2(_transform.position.x,newPosY);
				_newPosition.y = 0;						
			}
		}
		else
